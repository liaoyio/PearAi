...
const buttonVariants = cva(
  'inline-flex items-center gap-2 justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        default:
          'rounded-xl bg-primary-700 text-white hover:bg-primary-800 dark:hover:bg-primary-600 hover:shadow-sm',
        destructive:
          'rounded-full bg-red-600/90 dark:bg-red-600/70 border border-red-700 dark:border-red-600 text-white hover:bg-red-700 hover:shadow-sm',
        outline:
...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, BtnVariants {
  asChild?: boolean;
  isLoading?: boolean;
}
