...
const CARDS: Card[] = [
  ...
  },
  {
    ...
    description: (
      <>
        Continue is a leading open-source AI code assistant extension. PearAI Chat is built upon the
        foundation of Continue, with changes and a unified user experience. We acknowledge and
        respect Continue&apos;s contributions.
      </>
    ),
    icon: Continue,
...
export default function ExpandableCards() {
  ...
  const getCardWidth = (cardId: number) =>
    expandedId === null ? 'w-[185px]' : expandedId === cardId ? 'w-[380px]' : 'w-[185px]';

  return (
    <>
      {/* Desktop Layout */}
      <div className="hidden items-center justify-center px-6 py-4 lg:flex">
        <div className="w-full max-w-[1049px] rounded-xl border-[1.5px] px-5 py-8">
          <h1 className="pb-5 text-[28px] font-semibold leading-tight dark:text-gray-900">
            The AI space changes fast- PearAI Inventory curates the best AI tools on the market at
            any given time, and integrates them into a powerful editor.
          </h1>
                  ...
                  {expandedId !== card.id ? (
                    <>
                      ...
                      >
                        ...
                        >
                          <card.icon className="h-10 w-10" color={card.iconColor} />
                        </div>
                        <h1
                          style={{
