...
export default function Showcase() {
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      const newTotalPages = isMobileView ? testimonials.length : Math.ceil(testimonials.length / 2);
  ...
  const navigate = (direction: 'next' | 'previous') => {
    setCurrentPage((prevPage) =>
      direction === 'next' ? Math.min(prevPage + 1, totalPages - 1) : Math.max(prevPage - 1, 0),
    );
  };
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-center px-6 py-4">
        <div className="bg-white-50 z-10 max-w-3xl rounded-xl border-[1.5px] border-gray-200 p-5 dark:border-gray-50 dark:bg-black lg:max-w-[1049px]">
          ...
          <div className="relative overflow-hidden pb-5">
            <AnimatePresence initial={false} mode="wait">
              ...
              >
                {testimonials
                  .slice(currentPage * (isMobile ? 1 : 2), (currentPage + 1) * (isMobile ? 1 : 2))
